...
// 3. UI ANIMATIONS & ROUTING
document.addEventListener("DOMContentLoaded", () => { setTimeout(()=>document.body.classList.add("app-loaded"), 400); loadProds(); updateCart(); initCardTilt(); initScrollReveal(); });

window.go = id => {
  document.querySelectorAll(".page").forEach(p => { p.classList.remove("active"); p.style.opacity="0"; p.style.transform="translateY(12px)"; });
...
function initScrollReveal() {
  ...
  document.querySelectorAll(".card, .menu-item, .stat-box").forEach(el => { el.style.opacity="0"; el.style.transform="translateY(16px)"; el.style.transition="0.4s var(--ease-out-expo)"; obs.observe(el); });
}
...
function initCardTilt() {
  document.querySelectorAll(".card").forEach(card => {
    card.addEventListener("mousemove", e => {
      ...
      const xc = (x / r.width - 0.5) * 8; const yc = (y / r.height - 0.5) * -8;
      card.style.transform = `perspective(800px) rotateX(${yc}deg) rotateY(${xc}deg) translateY(-4px)`;
    });
...
async function loadProds() { try { const {data,error}=await window.sb.from("products").select("*").order("created_at",{ascending:false}); if(error) throw error; prods=data||[]; render(prods.slice(0,4)); initScrollReveal(); } catch(e){ window.toast("❌ Ошибка каталога","error"); } }
...
function render(list) {
  ...
  const c = document.getElementById("catalog-grid"); if(c) c.innerHTML = html || '<p style="grid-column:1/-1;text-align:center;padding:40px;color:var(--text-muted)">Ничего не найдено</p>';
...
window.addCart = id => {
  ...
  localStorage.setItem("cart",JSON.stringify(window.cart)); updateCart(); window.toast("✅ Добавлено","success");
};
...
window.loadOrders = async () => {
  ...
  try {
    ...
    if(!user) { c.innerHTML='<p style="text-align:center;padding:40px;color:var(--text-muted)">Войдите</p>'; return; }
    ...
    c.innerHTML = ordersRes.data?.length ? ordersRes.data.map(o=>{
      ...
      return `<div class="cart-item" style="display:block"><div style="display:flex;justify-content:space-between;margin-bottom:8px"><b>#${(o.id||"").slice(0,8)}</b><span>${o.total}</span></div><div style="color:var(--text-muted);margin-bottom:12px;font-size:0.9rem">${o.items}</div><div class="status-bar">${steps.map((_,i)=>`<div class="step ${i<=idx?'active':''}"></div>`).join("")}</div><div style="display:flex;justify-content:space-between;margin-top:8px;font-size:0.85rem;color:var(--text-muted)"><span>${labels[o.status]||o.status}</span><small>${new Date(o.created_at).toLocaleDateString("ru")}</small></div></div>`;
    }).join("") : '<p style="text-align:center;padding:40px;color:var(--text-muted)">Нет заказов</p>';
  } catch(e) { c.innerHTML='<p style="text-align:center;padding:40px;color:var(--danger)">Ошибка</p>'; }
};
...
async function loadAdmin() {
  try {
    ...
    document.getElementById("admin-prods").innerHTML=prods.map(p=>`<div style="display:flex;justify-content:space-between;padding:14px;border-bottom:1px solid var(--border)"><span>${p.name}</span><button onclick="window.delProd('${p.id}')" style="background:none;border:none;cursor:pointer;color:var(--danger)">🗑</button></div>`).join("")||'<p style="text-align:center;color:var(--text-muted)">Нет товаров</p>';
    const {data:allOrders}=await window.sb.from("orders").select("*").order("created_at",{ascending:false}).limit(10);
    const statuses=["new","processing","shipped","delivered","cancelled"];
    document.getElementById("admin-orders-list").innerHTML=allOrders?.map(o=>`<div style="display:flex;justify-content:space-between;align-items:center;padding:12px;border-bottom:1px solid var(--border)"><span>#${(o.id||"").slice(0,6)}<br><small style="color:var(--text-muted)">${o.user_email}</small></span><select onchange="window.updateOrderStatus('${o.id}',this.value)" class="input" style="width:auto">${statuses.map(s=>`<option value="${s}" ${s===o.status?'selected':''}>${s}</option>`).join("")}</select><button onclick="window.deleteOrder('${o.id}')" style="background:none;border:none;cursor:pointer;color:var(--danger)">🗑</button></div>`).join("")||'<p style="color:var(--text-muted)">Нет заказов</p>';
...
window.addProd = async () => {
  ...
  try {
    ...
    window.toast("✅ Опубликовано","success"); ["add-name","add-price","add-sizes","add-img","add-desc"].forEach(id=>{const el=document.getElementById(id);if(el)el.value=""}); if(f)document.getElementById("add-file").value=""; document.getElementById("img-preview").style.display="none"; await loadProds(); await loadAdmin();
...
// 9. AUTH & PROFILE
let isLogin=true;
document.querySelectorAll(".tab").forEach(t=>{t.onclick=()=>{document.querySelectorAll(".tab").forEach(x=>x.classList.remove("active"));t.classList.add("active");isLogin=(t.dataset.tab==="login");document.getElementById("auth-btn").textContent=isLogin?"Войти":"Регистрация";}});
...
window.sb.auth.onAuthStateChange(async(_,sess)=>{
  ...
  if(u){const{data:p}=await window.sb.from("profiles").select("*").eq("id",u.id).single();const n=p?.full_name||u.email.split("@")[0];document.getElementById("profile-display-name").textContent=n;document.getElementById("profile-email").textContent=u.email;document.getElementById("admin-menu").style.display=ADMIN_EMAILS.includes(u.email)?"flex":"none";}else{document.getElementById("profile-display-name").textContent="Гость";document.getElementById("profile-email").textContent="Войдите";document.getElementById("admin-menu").style.display="none";}
});
document.getElementById("logout-btn").addEventListener("click",async()=>{await window.sb.auth.signOut();window.currentUser=null;window.toast("👋 Вы вышли","info");});
window.editProfile=async()=>{if(!window.currentUser) return window.toast("Войдите","error");const{data:p}=await window.sb.from("profiles").select("*").eq("id",window.currentUser.id).single();document.getElementById("edit-fullname").value=p?.full_name||"";document.getElementById("edit-phone").value=p?.phone||"";document.getElementById("edit-avatar").value=p?.avatar_url||"";document.getElementById("profile-edit-modal").classList.add("open");};
window.closeProfileEdit=()=>document.getElementById("profile-edit-modal").classList.remove("open");
window.saveProfile=async()=>{if(!window.currentUser)return;const btn=event.target;btn.disabled=true;btn.textContent="⏳...";try{await window.sb.from("profiles").upsert({id:window.currentUser.id,email:window.currentUser.email,full_name:document.getElementById("edit-fullname").value.trim(),phone:document.getElementById("edit-phone").value.trim(),avatar_url:document.getElementById("edit-avatar").value.trim()});window.closeProfileEdit();document.getElementById("profile-display-name").textContent=document.getElementById("edit-fullname").value.trim()||window.currentUser.email.split("@")[0];window.toast("✅ Сохранено","success");}catch(e){window.toast("❌ "+e.message,"error");}finally{btn.disabled=false;btn.textContent="Сохранить";}};

// 10. REVIEWS
async function loadReviews(pid){const l=document.getElementById("reviews-list");if(!l)return;try{const{data:r}=await window.sb.from("reviews").select("*,profiles(full_name)").eq("product_id",pid).order("created_at",{ascending:false});l.innerHTML=r?.length?r.map(x=>`<div class="review-card"><div style="display:flex;justify-content:space-between"><b>${x.profiles?.full_name||x.user_email.split("@")[0]}</b><span style="color:#f59e0b">${"⭐".repeat(x.rating)}</span></div><p style="margin-top:8px;color:var(--text-muted)">${x.comment}</p>${window.currentUser&&ADMIN_EMAILS.includes(window.currentUser.email)?`<button onclick="window.deleteReview('${x.id}')" style="background:none;border:none;color:var(--danger);cursor:pointer;margin-top:8px;font-size:0.8rem">Удалить</button>`:''}</div>`).join(""):'<p style="color:var(--text-muted)">Нет отзывов</p>';}catch(e){console.error(e);}}
