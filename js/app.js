...
window.go = (id) => {
  ...
  const btn = document.querySelector(`.nav-item[onclick*="${id}"]`); btn?.classList.add('active');
  window.scrollTo(0,0);
...
document.querySelectorAll('.cat-btn').forEach(b => b.onclick = () => {
  ...
  render(prods.filter(p => c==='all' || p.category===c));
});
document.getElementById('search-input')?.addEventListener('input', e => {
  const q = e.target.value.toLowerCase();
  render(prods.filter(p => p.name.toLowerCase().includes(q) || p.category.includes(q)));
});
...
function render(list) {
  ...
  g.innerHTML = list.map(p => `
    <div class="card" onclick="window.openProd('${p.id}')">
      ...
      <div class="card-body">
        <div class="card-brand">${p.category}</div>
...
window.openProd = (id) => {
  ...
  document.getElementById('sizes-container').innerHTML = (p.sizes||[]).map(s => `<button class="size-btn" onclick="this.parentElement.querySelectorAll('.size-btn').forEach(b=>b.classList.remove('active'));this.classList.add('active'); window._sz='${s}'">${s}</button>`).join('');
  window.go('product');
};
...
window.addCart = (id) => {
  ...
  updateCart(); alert('✅ Добавлено');
};
window.addToCartFromDetail = () => {
  if(!window._cur || !window._sz) return alert('Выбери размер');
...
function updateCart() {
  ...
  document.getElementById('cart-items').innerHTML = window.cart.map((i,k) => `
    <div class="cart-item">
      <div class="cart-item-name">${i.name} (${i.size||'?'})</div>
      <div class="cart-controls"><button onclick="chgQ(${k},-1)">−</button><span>${i.qty||1}</span><button onclick="chgQ(${k},1)">+</button><button style="color:red" onclick="rmQ(${k})">🗑</button></div>
    </div>`).join('');
...
window.checkout = async () => {
  ...
  try {
    const {data:o, error} = await window.sb.from('orders').insert({
      user_email: user.email, user_name: user.email.split('@')[0],
      items: window.cart.map(i=>`${i.name} (${i.size||'?'}) x${i.qty}`).join(', '),
      total: total.toLocaleString('ru')+' ₽', address: addr, status: 'new'
    }).select().single();
    ...
    fetch(`https://api.telegram.org/bot${TG_TOKEN}/sendMessage`, {
      ...
      body:JSON.stringify({chat_id:TG_CHAT, text:`📦 Заказ\n👤 ${o.user_email}\n🛍️ ${o.items}\n💰 ${o.total}\n📍 ${addr}`, parse_mode:'HTML'})
    }).catch(console.error);
...
window.loadOrders = async () => {
  ...
  c.innerHTML = data?.length ? data.map(o => `
    <div style="background:#fff;padding:12px;margin-bottom:10px;border-radius:8px;box-shadow:0 2px 6px rgba(0,0,0,0.1)">
      <div style="display:flex;justify-content:space-between"><b>#${o.id.slice(0,8)}</b><span>${o.status}</span></div>
      <div style="font-size:0.9rem;margin:4px 0">${o.items}</div>
      <div>${o.address}</div>
      <div style="font-weight:700">${o.total}</div>
      ${o.qr_image_url?`<img src="${o.qr_image_url}" style="max-width:150px;margin-top:8px;border-radius:6px">`:''}
    </div>`).join('') : '<p style="text-align:center;color:#888">Нет заказов</p>';
};
...
async function loadAdmin() {
  ...
  list.innerHTML = prods.map(p => `
    <div style="display:flex;justify-content:space-between;padding:8px;border-bottom:1px solid #eee">
      ...
      <button onclick="window.delProd('${p.id}')" style="color:red;background:none;border:none;cursor:pointer">🗑</button>
    </div>`).join('');
}

window.addProd = async () => {
  const n = document.getElementById('add-name').value;
  const p = Number(document.getElementById('add-price').value);
  if(!n||!p) return alert('Заполни имя и цену');
  const d = {
    name: n, price: p,
    category: document.getElementById('add-cat').value,
    sizes: document.getElementById('add-sizes').value.split(',').map(s=>s.trim()),
    image_url: document.getElementById('add-img').value,
    description: document.getElementById('add-desc').value
  };
  const btn = event.target; btn.disabled=true; btn.textContent='⏳...';
  try {
    await window.sb.from('products').insert(d);
    alert('✅ Добавлено');
...
window.savePVZ = () => { window.pvz = {city:document.getElementById('pvz-city').value, addr:document.getElementById('pvz-addr').value}; localStorage.setItem('pvz',JSON.stringify(window.pvz)); document.getElementById('pvz-modal').style.display='none'; alert('✅'); };

window.openChat = () => { document.getElementById('chat-modal').style.display='flex'; const b=document.getElementById('chat-body'); b.innerHTML='<div style="background:#f0f0f0;padding:10px;border-radius:8px;align-self:flex-start">👋 Привет! Чем помочь?</div>'; };
...
window.sendMsg = () => {
  ...
  setTimeout(()=>b.innerHTML+=`<div style="background:#f0f0f0;padding:10px;border-radius:8px;align-self:flex-start">🤔 Оператор ответит скоро.</div>`,500);
