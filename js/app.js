...
function renderAdmin() {
  ...
  if(auth.currentUser.email !== 'maslakov.antoni@yandex.ru') return; 
...
authForm.onsubmit=async e=>{
  ...
  try {
    ...
    const msgs = {'auth/user-not-found':'Пользователь не найден.','auth/wrong-password':'Неверный пароль.','auth/email-already-in-use':'Email уже занят.','auth/invalid-email':'Неверный email.','auth/weak-password':'Минимум 6 символов.'};
