...
window.openProd = async id => {
  ...
  const b=document.getElementById("detail-brand"); if(b) b.textContent=p.category;
  const n=document.getElementById("detail-name"); if(n) n.textContent=p.name;
  const pr=document.getElementById("detail-price"); if(pr) pr.textContent=Number(p.price).toLocaleString("ru")+" ₽";
  const d=document.getElementById("detail-desc"); if(d) d.textContent=p.description||"";
  const sizes=Array.isArray(p.sizes)?p.sizes:(p.sizes||"").split(",").map(s=>s.trim()).filter(Boolean);
  const sz=document.getElementById("sizes-container"); if(sz) sz.innerHTML=sizes.length?sizes.map(s=>`<button class="size-btn" onclick="document.querySelectorAll('.size-btn').forEach(b=>b.classList.remove('active'));this.classList.add('active');window._sz='${s}'">${s}</button>`).join(""):'<span style="color:var(--text-muted)">Нет данных</span>';
  window.go("product"); await loadReviews(id);
};

// 5. CART
window.addCart = id => { const p=prods.find(x=>x.id===id); if(!p) return; const ex=window.cart.find(x=>x.id===id && x.size===window._sz); if(ex) ex.qty++; else window.cart.push({...p, qty:1, size:window._sz||null}); localStorage.setItem("cart",JSON.stringify(window.cart)); updateCart(); window.toast("✅ В корзине","success"); };
...
function updateCart() {
  ...
  const items=document.getElementById("cart-items"); if(items) items.innerHTML=window.cart.map((i,k)=>`<div class="cart-item"><div><b>${i.name}</b>${i.size?`<br><small style="color:var(--text-muted)">Размер: ${i.size}</small>`:""}</div><div class="cart-controls"><button onclick="chgQ(${k},-1)">−</button><span>${i.qty}</span><button onclick="chgQ(${k},1)">+</button><button onclick="rmQ(${k})" style="color:var(--danger);background:none;border:none">🗑</button></div></div>`).join("");
  const t=document.getElementById("cart-total"); if(t) t.textContent=window.cart.reduce((s,i)=>s+Number(i.price)*(i.qty||1),0).toLocaleString("ru")+" ₽";
}
...
window.editProd = id => {
  ...
  document.getElementById("add-cat").value = p.category;
  document.getElementById("add-sizes").value = Array.isArray(p.sizes)?p.sizes.join(","):p.sizes||"";
